...
static inline void host2hexagon_(buffer_config_t *in, hexagon_buffer_config_t *out) {
...
static inline void host2hexagon(effect_config_t *in, hexagon_effect_config_t *out) {
    ...
    host2hexagon_(&in->inputCfg, &out->inputCfg);
    host2hexagon_(&in->outputCfg, &out->outputCfg);
}

static inline void hexagon2host_(hexagon_buffer_config_t *in, buffer_config_t *out) {
    if (in->mask & EFFECT_CONFIG_SMP_RATE) {
        out->mask |= EFFECT_CONFIG_SMP_RATE;
        out->samplingRate = in->samplingRate;
    }
...
static inline void hexagon2host(hexagon_effect_config_t *in, effect_config_t *out) {
    memset(out, 0, sizeof(*out));
    hexagon2host_(&in->inputCfg, &out->inputCfg);
    hexagon2host_(&in->outputCfg, &out->outputCfg);
}
